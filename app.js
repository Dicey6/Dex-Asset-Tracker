...
// ─── Constants ───────────────────────────────────────────────────────────────
const SAMPLE_TOKEN = '8hHfn2TX7h6BYFeVtjT6kr3b8LKRhqwBAtNuKNJepump';
...
function renderWalletAnalysis(data) {
  ...
  setText('[data-wallet-sol-usd]', portfolio.solValueUsd !== null && portfolio.solValueUsd !== undefined ? `${money(portfolio.solValueUsd)} included in total` : 'USD value unavailable');
  ...
  if (holdingsEl) {
    holdingsEl.innerHTML = holdings.length ? `
      <div class="holder-table-head wallet-holdings-head">
         <span>#</span><span>Token</span><span>Balance</span><span>Value (USD)</span><span>PnL</span><span>Allocation</span><span></span>
      </div>
      ${holdings.map((h, i) => {
        ...
        return `<div class="holder-row wallet-holdings-row">
          ...
          <span class="mono">${escHtml(money(h.valueUsd))}</span>
           <span class="holding-pnl ${pnlClass(h.pnlPercent)}">${h.pnlPercent !== null && h.pnlPercent !== undefined ? escHtml(pct(h.pnlPercent)) : '—'}</span>
  ...
  // One combined activity table: swaps, trades, transfers, and provider history.
  const activityEl = $('[data-wallet-activity]');
  if (activityEl) {
    const rows = [
      ...activity.map((a) => ({ kind: a.type, asset: a.description, amount: null, timestamp: a.timestamp, signature: a.signature })),
      ...swaps.map((swap) => ({ kind: swap.side || 'SWAP', asset: swap.tokenSymbol || shortAddr(swap.tokenMint), amount: swap.volumeUsd !== null ? money(swap.volumeUsd) : '—', timestamp: swap.timestamp, signature: swap.signature })),
      ...transfers.map((transfer) => ({ kind: transfer.direction === 'in' ? 'RECEIVE' : 'SEND', asset: shortAddr(transfer.tokenMint) || 'SOL', amount: transfer.amount !== null ? count(transfer.amount) : '—', timestamp: transfer.timestamp, signature: transfer.signature })),
    ].filter((row, index, all) => all.findIndex((candidate) => candidate.signature === row.signature) === index)
      .sort((a, b) => new Date(b.timestamp || 0).getTime() - new Date(a.timestamp || 0).getTime())
      .slice(0, 20);
    activityEl.innerHTML = rows.length ? `
      <div class="holder-table-head wallet-activity-head"><span>#</span><span>Type</span><span>Native ticker / token</span><span>Amount</span><span>Time</span><span></span></div>
      ${rows.map((row, index) => {
        const rawType = String(row.kind || '').toLowerCase();
        const type = rawType.includes('buy') ? 'BUY' : rawType.includes('sell') ? 'SELL' : rawType.includes('transfer') || rawType.includes('receive') || rawType.includes('send') ? (rawType.includes('receive') ? 'RECEIVE' : 'SEND') : rawType.includes('swap') ? 'SWAP' : 'ACTIVITY';
        const timestamp = row.timestamp ? Math.floor(new Date(row.timestamp).getTime() / 1000) : null;
        return `
        <div class="holder-row wallet-activity-row">
          <span class="rank">${String(index + 1).padStart(2, '0')}</span>
          <span class="activity-type ${type.toLowerCase()}">${type}</span>
          <span class="mono">${escHtml(row.asset || '—')}</span>
          <span class="mono">${escHtml(row.amount || '—')}</span>
          <span class="mono time-cell">${IC.clock} ${escHtml(timestamp ? relativeTime(timestamp) : '—')}</span>
          <span class="row-actions"><a class="icon-action" href="https://solscan.io/tx/${encodeURIComponent(row.signature)}" target="_blank" rel="noreferrer" title="View transaction">${IC.ext}</a></span>
        </div>`;
      }).join('')}
    ` : '<div class="empty-copy"><b>No recent activity</b><span>Transaction history needs a configured provider or the wallet has no indexed activity.</span></div>';
  }
...
function renderWatchlist(items = [], profile = null) {
  ...
  target.querySelectorAll('[data-remove-watch]').forEach((button) => button.addEventListener('click', async () => {
    ...
    const client = await getSupabase();
    if (!row || !client || !_authSession) return;
    const result = await client
      .from('watchlists')
      .update({ is_active: false })
      .eq('id', row.dataset.watchId)
...
async function watchCurrentToken() {
  ...
  try {
    ...
    const token = _currentTokenData.token || {};
    const marketCapValue = token.marketCap;
    const priceValue = token.priceUsd;
    const marketCap = marketCapValue === null || marketCapValue === undefined || marketCapValue === ''
      ? null
      : Number.isFinite(Number(marketCapValue)) ? Number(marketCapValue) : null;
    const price = priceValue === null || priceValue === undefined || priceValue === ''
      ? null
      : Number.isFinite(Number(priceValue)) ? Number(priceValue) : null;
    const payload = {
      user_id: _authSession.user.id,
      token_mint: String(_currentTokenData.address || '').trim(),
      token_symbol: String(token.symbol || '').trim() || null,
      token_name: String(token.name || '').trim() || null,
      current_market_cap: marketCap,
      market_cap_at_watch: marketCap,
    ...
    if (!payload.token_mint) throw new Error('The analyzed token has no mint address.');

    // The unique constraint prevents duplicate rows. Ignoring conflicts keeps
    // an existing row's original market-cap baseline intact.
    const result = await client
      .from('watchlists')
      .upsert(payload, { onConflict: 'user_id,token_mint', ignoreDuplicates: true });
    if (result.error) throw result.error;

    // Re-enable an old inactive row without resetting its reference baseline.
    const reactivated = await client
      ...
      .update({
        ...
        current_market_cap: payload.current_market_cap,
      })
      .eq('user_id', _authSession.user.id)
