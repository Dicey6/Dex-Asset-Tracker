...
async function solUsdPrice() {
  const body = await fetchJson<JsonRecord>(`https://api.dexscreener.com/latest/dex/tokens/${WRAPPED_SOL}`);
  const pairs = rows(body);
  return pairs
    .map((pair) => firstNumber(pair.priceUsd))
...
export default async function handler(request: ApiRequest, response: ApiResponse) {
  ...
  const [publicResult, heliusResult, solscanTokenResult, solscanActivityResult, birdeyeResult, heliusActivityResult, birdeyeSwapResult, solPriceResult] = await Promise.all([
  ...
  const holdings: WalletHolding[] = Array.from(new Map<string, WalletHolding>(sourceHoldings.map((item) => [item.address, item])).values())
    .sort((a, b) => (b.valueUsd ?? 0) - (a.valueUsd ?? 0))
    .slice(0, 20);

  const solscanActivityRows: WalletActivity[] = rows(solscanActivityResult.value).slice(0, 10).map((item) => ({
  ...
  const activity: WalletActivity[] = solscanActivityRows.length ? solscanActivityRows : heliusActivityRows.slice(0, 10).map((item) => ({
    signature: String(item.signature ?? '—'),
  ...
  const holdingsWithPnl = holdings.map((holding) => {
    ...
    const pnlPercent = result.avgBuyPrice !== null && currentPrice !== null && result.avgBuyPrice > 0
      ? ((currentPrice - result.avgBuyPrice) / result.avgBuyPrice) * 100
      : null;
    return { ...holding, pnlPercent };
  });
  const pnlRows = holdingsWithPnl.map((holding) => {
    const currentPrice = holding.balance && holding.valueUsd !== null ? holding.valueUsd / holding.balance : null;
    const result = computeBuyerPnl(rows(birdeyeSwapResult.value), holding.address, holding.balance, currentPrice);
  ...
  const solPriceUsd = solPriceResult.value ?? null;
  const solValueUsd = normalizedSol !== null && solPriceUsd !== null ? normalizedSol * solPriceUsd : null;
  const tokenValueUsd = holdingsWithPnl.reduce((sum, holding) => sum + (holding.valueUsd ?? 0), 0);
  const totalValueUsd = tokenValueUsd + (solValueUsd ?? 0) > 0 ? tokenValueUsd + (solValueUsd ?? 0) : null;
  const topHoldingPercentage = totalValueUsd ? ((holdingsWithPnl[0]?.valueUsd ?? 0) / totalValueUsd) * 100 : null;
  const notes = [
